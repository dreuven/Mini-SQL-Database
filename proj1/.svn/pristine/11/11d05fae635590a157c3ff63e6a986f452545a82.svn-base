...
class CommandInterpreter {
    ...
    Table tableDefinition() {
        ...
        if (_input.nextIf("(")) {
            ...
            columntitlesList.add(name);
            if (_input.nextIs(",")) {
                while (!_input.nextIs(")")) {
                    _input.next(",");
                    String nameTwo = columnName();
                    columntitlesList.add(nameTwo);
                }
            }
            _input.next(")");
            table = new Table(columntitlesList);
        } else {
            _input.next("as");
            table = selectClause();
        }
    ...
    Table selectClause() {
        ...
        if (_input.nextIs(",")) {
            ...
            if (_input.nextIs(";")) {
                return tab1.select(tableTwo, columnNames, null);
            } else {
                _input.next("where");
                ArrayList<Condition> myConds = conditionClause(tab1, tableTwo);
                return tab1.select(tableTwo, columnNames, myConds);
            }
        } else {
            if (_input.nextIs(";")) {
                return tab1.select(columnNames, null);
            } else {
                _input.next("where");
                ArrayList<Condition> myConds = conditionClause(tab1);
                return tab1.select(columnNames, myConds);
            }
        }
    ...
    ArrayList<Condition> conditionClause(Table... tables) {
        ...
        while (!_input.nextIs(";")) {
            ...
            Condition newcond = condition(tables);
    ...
    Condition condition(Table... tables) {
        ...
        Column col = new Column(name, tables);
        if (_input.nextIs(Tokenizer.LITERAL)) {
            String value = literal();
            Condition cond = new Condition(col, relation, value);
            return cond;
        } else {
            String name2 = name();
            Column col2 = new Column(name2, tables);
            Condition cond = new Condition(col, relation, col2);
            return cond;
        }
    }
